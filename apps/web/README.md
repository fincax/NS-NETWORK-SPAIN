...
src/core        dominio puro, sin framework: types (zod), scoring (puertas + NS Match Score + Explanation),
                compliance (Salvoconducto), state-machine (Cesión), merit (Promesa, Veredicto, Mérito), aval (Eco y Aval, D-042), rulebook (Reglamento: Normas y Ventajas, D-043)
...
src/app         / (portada beta pública) · acceso (puerta de la demo) · eco/[token] (página pública del Interesado, D-042) · aval/[slug] (Aval público del titular, sin datos de contacto) · (app)/: hoy · mesa · cesiones/[id] · indicio · sala · sala/reglamento · empresa/[slug]
