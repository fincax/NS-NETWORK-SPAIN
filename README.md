...
| [`docs/08_SECURITY_PRIVACY_GDPR.md`](./docs/08_SECURITY_PRIVACY_GDPR.md) | Seguridad, privacidad y RGPD: lo que hay (aviso de privacidad y consentimiento versionado en la candidatura, D-055) y lo pendiente antes de empresas reales. |
...
| [`docs/17_DESPLIEGUE.md`](./docs/17_DESPLIEGUE.md) | Cuándo y cómo pasar al servidor: entorno privado de demostración ahora, producción con empresas reales tras la puerta de Fase 1. Paso a paso para Vercel (§4) y para un servidor propio en Clouding con `deploy/instalar.sh`, `deploy/actualizar.sh` y las copias de seguridad de `deploy/copias.sh` (§4c). |
| [`docs/18_GUIA_DEL_TIMONEL.md`](./docs/18_GUIA_DEL_TIMONEL.md) | Guía del Timonel para empresas candidatas: qué es NS, cómo se entra, la semana del Timonel, la Cesión a dos caras, las siete Normas NS, privacidad por capas, cuota por Tramos, glosario y preguntas frecuentes. |
